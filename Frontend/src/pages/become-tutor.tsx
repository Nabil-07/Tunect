...
export default function BecomeTutor() {
  ...
  const startApplication = async () => {
    ...
    setIsLoading(true);
    try {
      // Optimized: Try profiles route first, fallback to auth route only if needed
      ...
      // Only try fallback if profiles route doesn't exist (404) or is unavailable
      if (err?.response?.status === 404 || err?.response?.status >= 500) {
        try {
          const fallbackResponse = await http.post("/auth/choose-role", { role: "TUTOR" });
          if (fallbackResponse?.data?.access_token) {
            setTokens({ accessToken: fallbackResponse.data.access_token });
          }
          localStorage.setItem("role", "TUTOR");
          navigate("/tutor/kyc", { replace: true });
        } catch (fallbackErr) {
          console.error('Failed to choose TUTOR role:', fallbackErr);
          // Still navigate to KYC if user is already logged in
          localStorage.setItem("role", "TUTOR");
          navigate("/tutor/kyc", { replace: true });
        }
      } else {
    ...
    <div className="bg-white text-slate-800 relative">
      ...
      {isLoading && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-white/90 backdrop-blur-sm">
          <Loader fullScreen={false} size="lg" message="Setting up your tutor account..." />
        </div>
      )}
      {/* Hero Section */}
