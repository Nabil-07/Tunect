...
async function main() {
  // 1. Check user records (including soft-deleted via raw query)
  ...
  // 2. Check transactional data preservation
  ...
  // 3. Check tutor transactional data
  ...
  const wallet1: any[] = await p.$queryRawUnsafe(`SELECT balance FROM "TutorWallet" WHERE "tutorId" = 'seed_tutor_001'`);
  const wallet2: any[] = await p.$queryRawUnsafe(`SELECT balance FROM "TutorWallet" WHERE "tutorId" = 'seed_tutor_002'`);
  ...
  const bookingStatuses: any[] = await p.$queryRawUnsafe(`
